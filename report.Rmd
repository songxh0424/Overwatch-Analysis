...
## Does meta heroes perform better than others?
...
p = ggplot(ggdat, aes(class, `Time Played`, group = hero, fill = class)) +
  ...
  geom_text(aes(label = hero, color = difficulty), position = position_dodge(0.9),
            angle = 90, size = 1.8, hjust = -0.2, show.legend = FALSE) +
  ylim(c(0, 12e6)) + geom_hline(yintercept = 3000000, color = 'grey78', linetype = 'dotted') +
...
the "off-meta" heroes are prone to lose. Some of the less-picked heroes like Hanzo Doomfist and Reaper indeed have low win rates,
...
distance = dist(stats)
fit1 = kmeans(stats, 3)
sil1 = silhouette(fit1$cluster, distance)
plot(sil1)
result1 = heroUsage.wide %>% mutate(cluster = fit$cluster)

fit2 = kmeans(stats, 2)
sil2 = silhouette(fit2$cluster, distance)
plot(sil2)
result2 = heroUsage.wide %>% mutate(cluster = fit$cluster)

pcaScores = pca$scores[, 1:2]
dist.pca = dist(pcaScores)
fit4 = kmeans(pcaScores, 2)
sil4 = silhouette(fit4$cluster, dist.pca)
plot(sil4)
ggplot(data.frame(pcaScores), aes(Comp.1, Comp.2, color = fit4$cluster)) + geom_point()

## pam
fit3 = pam(distance, 2, diss = TRUE)
sil3 = silhouette(fit3$clustering, distance)
plot(sil3)
```
