...
Another aspecAnother aspect of hero usage is to look at how players' attribute their total play time with each hero. For example, some
...
t of hero usage is to look at how players' attribute their total play time with each hero. For example, some
players are good at switching heroes mid match to counter their opponents, while other players can only play one or two heroes
...
From Figure 3, we can see that a large portion of Mercy mains indeed spend majority of their time playing only Mercy, as compared
...
across different SR levels. The results are shown as scatter plots in Figure 4. The lines in the scatter plots are produced using
...
Based on Figure 4, we can make the following observations:
